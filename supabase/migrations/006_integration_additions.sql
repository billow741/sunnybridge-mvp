...
-- 基于 MVP 10 张表主干，仅 ADD COLUMN + 新建 3 张表
--
-- 核心原则：
...
-- ============================================================
-- Part A: ALTER TABLE — 扩展现有 3 张表（不改列、不删列、不改表名）
-- ============================================================
...
ALTER TABLE children ADD COLUMN IF NOT EXISTS total_hours INTEGER NOT NULL DEFAULT 0
  CHECK (total_hours >= 0);
ALTER TABLE children ADD COLUMN IF NOT EXISTS used_hours INTEGER NOT NULL DEFAULT 0
  CHECK (used_hours >= 0);
...
ALTER TABLE courses ADD COLUMN IF NOT EXISTS package_id UUID
  REFERENCES packages(id) ON DELETE SET NULL;  -- 可空兼容字段，当前不依赖

-- courses.status 扩充 CHECK 支持 'absent'（CRM 有此状态）
-- PG 不能直接 ALTER CHECK，需要 DROP 旧约束再 ADD 新约束
...
-- ============================================================
-- Part B: CREATE TABLE — 新建 3 张核心表
-- （packages 预留但 P0 不参与业务逻辑，payments/teacher_payments/settings 正式使用）
-- ============================================================
...
-- ── C2. RLS 策略（全部 deny anon，仅 service_role 可操作）──
...
RETURNS TRIGGER AS $$
BEGIN
    -- 仅当 status 从非completed 变为 completed 时触发
...
-- ============================================================
-- Part D: 迁移映射表（CRM INTEGER id → MVP UUID）
-- ============================================================
