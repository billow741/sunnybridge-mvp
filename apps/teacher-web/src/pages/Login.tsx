...
export default function Login() {
  ...
  const onFinish = async (values: { username: string; password: string }) => {
    setLoading(true);
    try {
      const { data } = await client.post('/auth/teacher/login', values);
      setAuth(data.access_token, { username: values.username, name: values.username }, data.must_change_password);
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #5CAADF 0%, #F4A230 100%)' }}>
      <div style={{ width: 380, padding: '40px 32px', background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: 'linear-gradient(135deg, #5CAADF, #F4A230)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, color: '#fff', fontWeight: 700, marginBottom: 12 }}>S</div>
          <h2 style={{ margin: 0, fontSize: 22, color: '#1e293b' }}>SunnyBridge</h2>
          <p style={{ color: '#64748b', margin: '4px 0 0' }}>教师端登录</p>
        </div>
