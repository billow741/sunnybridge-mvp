/**
 * ReadingPage — A-READING (ADMIN-05).
 *
 * Features:
 * - Paginated reading material list (Ant Design Table)
 * - Level / Category / Active status filters
 * - Create material → Modal form → pdf_url auto-filled as pending_upload
 * - Edit material → Modal form (pre-filled)
 * - Delete material → Popconfirm → hard delete
 ...
 * API: consumes API-08 endpoints via services/reading.ts.
 */
...
const ReadingPage: React.FC = () => {
  ...
  // ── Upload ref (material id set before upload starts) ──
    ...
    try {
      if (editingMaterial) {
        ...
        const result = await createMaterial({
          ...
          page_count: values.page_count ?? 0,
        ...
        setFormOpen(false);
        fetchList();
        // Auto-open upload modal for the newly created material
  ...
  const handleToggleActive = async (record: ReadingMaterial, checked: boolean) => {
    try {
      await updateMaterial(record.id, { is_active: checked });
  ...
  const columns: ColumnsType<ReadingMaterial> = [
    ...
    },
    {
      ...
      key: 'category',
      width: 80,
      render: (category: string) => CATEGORY_LABEL_MAP[category] ?? category,
    },
    {
      title: '页数',
      ...
      key: 'page_count',
      width: 70,
      render: (count: number) => (count > 0 ? count : <Text type="secondary">—</Text>),
    },
    {
      title: '排序',
      ...
      render: (_: unknown, record: ReadingMaterial) => (
        <Space size="small" wrap>
          ...
          <Button
            ...
            icon={<UploadOutlined />}
            onClick={() => {
              uploadingMaterialIdRef.current = record.id;
          ...
          {record.is_active ? (
            <Popconfirm
              title="确定下架该阅读材料吗？"
              description="下架后 App 端将不再展示此材料，已有阅读进度不受影响"
          ...
          <Popconfirm
            title="确定删除该阅读材料吗？"
            description="删除后不可恢复，关联的阅读进度也会被清除"
